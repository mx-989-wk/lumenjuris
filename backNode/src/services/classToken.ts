...
export class Token {
    ...
    async createToken(userId: number, type: TokenType) {
        ...
        try {
            ...
            if (!expiresAt) {
                throw new Error(`Echec lors de la création d'un nouveau token, aucun expiresAt définit pour le type "${type}".`)
            }
            ...
            return {
                success: false,
                message: "Une erreur est survenue avec le serveur lors de la création d'un nouveau token."
            }
        }
    }
    ...
    async managementDeleteTokenExpires() {
        try {
            ...
            return {
                success: false,
                message: "Une erreur est survenue lors du management des tokens expirés."
            }
        }
    }
}
